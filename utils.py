...
class Logger(object):
    ...
    def __init__(self, fn, root="./logs", subdir=None, resume=None):
        ...
        if resume:
            ...
            logdir = self._make_dir(fn, root, subdir)
    ...
    def _make_dir(self, fn, root, subdir):
        if subdir is None:
            subdir = datetime.today().strftime("%y%m%d")
        #prefix = f'{subdir}/{np.random.randint(10000)}_'
        logdir = f'{root}/{subdir}/{fn}/{np.random.randint(10000)}'
        return logdir
