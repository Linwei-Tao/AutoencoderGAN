...
class BaseDiscriminator(nn.Module, metaclass=ABCMeta):
    def __init__(self, d_penul, n_classes=1, d_hidden=128, d_project=128,
        ...
        # self.projectionbeforez = nn.Sequential(
        #     nn.Linear(d_penul, d_penul),
        #     nn.LeakyReLU(0.1, inplace=True),
        # )
    ...
    def forward(self, inputs, y=None,
                ...
                finetuning=False, sg_linear=False, projectionbeforez=False, return_feature_maps=False):
        _aux = {}
        _return_aux = False
        ...
        if finetuning:
            ...
            features, feature_maps = self.penultimate(inputs, return_feature_maps)
        ...
        if projectionbeforez:
            features = self.projectionbeforez(features)
        ...
        if _return_aux and return_feature_maps:
            return output, _aux, feature_maps
        elif _return_aux and not return_feature_maps:
            return output, _aux
        elif not _return_aux and return_feature_maps:
            return output, feature_maps
        elif not _return_aux and not return_feature_maps:
            return output
