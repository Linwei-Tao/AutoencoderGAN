...
from training.criterion import LapLoss, VGGPerceptualLoss
...
def parse_args():
    ...
    parser.add_argument('--logdir', help='Path to log root', default='./logs', type=str)
    ...
    # Options for Debiased Contrastive Discriminator (DCD)
    parser.add_argument('--no_debiase', action='store_false',
                        help='Do not debiase DCD loss')
    parser.add_argument('--tau_plus', default=0.1, type=float,
                        help='biased samples probability in real samples ')
    parser.add_argument('--tau_plus4gen', default=0.05, type=float,
                        help='biased samples probability in generated samples ')
    parser.add_argument('--beta4r', default=1, type=float,
                        help='HCL beta for real samples')
    parser.add_argument('--beta4g', default=0.5, type=float,
                        help='HCL beta for generated samples')
    parser.add_argument('--M', default=1, type=int,
                        help='Number of positve compension pairs')

    # Options for Autoencoder
    parser.add_argument('--mse_factor_on_z', default=0.01, type=float,
                        help='Control the mse loss between generated samples based on augumentation representations and real samples')
    parser.add_argument('--mse_factor_on_img', default=0.01, type=float,
                        help='Control the mse loss between generated samples based on augumentation representations and real samples')
    parser.add_argument('--dis_genral_factor', default=0.01, type=float,
                        help='Control the discrimination loss between generated samples and augumentation')
    parser.add_argument('--z_constrain_factor', default=0.01, type=float,
                        help='Control the mse loss between generated samples and augumentation')
...
def train(P, opt, train_fn, models, optimizers, train_loader, logger):
    ...
    for step in range(P.starting_step, opt['max_steps']+1):
        ...
        gen_images, latent_samples = _sample_generator(generator, images.size(0))
        g_loss = train_fn["G"](P, discriminator, opt, images, gen_images, latent_samples, generator)
...
def worker(gpu, P):
    ...
    if P.rank == 0:
        if P.resume:
            ...
            logger = Logger(f'{P.filename}{P.comment}', root=P.logdir, subdir=f'gan/{P.gin_stem}/{P.architecture}')
