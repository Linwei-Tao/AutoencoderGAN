...
def loss_D_fn(P, D, options, images, gen_images, G, loss_fn):
    ...
    d_all, aux, real_feature_maps = D(aug_images, sg_linear=True, projection=True, projection2=True, return_feature_maps=True)
    ...
    # return simclr_loss + P.lbd_a * sup_loss + P.D_view_constrain * laploss_loss_on_img, {
    return simclr_loss + P.lbd_a * sup_loss + P.D_view_constrain * perceptual_loss_on_img, {
    # return simclr_loss + P.lbd_a * sup_loss + P.D_view_constrain * mse_loss_on_z, {
       # + P.D_Dis*(d_gen_real1_loss+d_gen_real2_loss)\
...
def loss_G_fn(P, D, options, images, gen_images, latent, G):
    ...
    return g_loss + P.G_z_constrain * F.mse_loss(latent, aux['projection'])