...
def loss_D_fn(P, D, options, images, gen_images):
    ...
    d_all, aux = D(P.augment_fn(cat_images), sg_linear=True, projection=True, projection2=True, projection3=True)

    # simclr_loss for fake
    views = aux['projection']
    views = F.normalize(views)
    genview1, genview2 = views[2 * N:3*N],views[3*N:],
    simclr4fake_loss = nt_xent(genview1, genview2, temperature=P.temp, distributed=P.distributed, normalize=False)
    ...
    return debiased_loss+simclr4fake_loss, {
        "penalty": d_loss,
        "d_real": d_real.mean(),
        "d_gen": d_gen.mean(),
    }
