...
def loss_D_fn(P, D, options, images, gen_images, G):
    ...
    mse_loss = mse(gen_real1, aug_images[:N]) + mse(gen_real2, aug_images[N:2*N])

    # #compare generated real samples and original samples
    # d_all, aux = D(aug_images, sg_linear=True, projection=True, projection2=True)
    # d_gen_real1, d_gen_real2 = d_all[:N], d_all[N:2 * N]
    ...
    return simclr_loss + P.lbd_a * sup_loss + P.mse_factor*mse_loss, {
        "penalty": d_loss,
        "d_real": d_real.mean(),
        "d_gen": d_gen.mean(),
    }
...
def loss_G_fn(P, D, options, images, gen_images, G):
