...
def debiased_log_softmax(sim_matrix, tau_plus, N, temperature):
    ...
    Ng = (-tau_plus * (2*N-2+N) * pos + neg_real + neg_fake) / (1 - tau_plus)
    # constrain (optional)
    Ng = torch.clamp(Ng, min=(2*N-2+N) * np.e ** (-1 / temperature))
    lsm = torch.log(pos / (pos + Ng))/(2*N-1)
    lsm=lsm.fill_diagonal_(0)
    return lsm

def debiased_con_loss(real1, real2, fake, temperature, distributed=False):
    if distributed:
        real1 = torch.cat(GatherLayer.apply(real1), dim=0)
        real2 = torch.cat(GatherLayer.apply(real2), dim=0)
        fake1 = torch.cat(GatherLayer.apply(fake), dim=0)
    N = real1.size(0)
...
def loss_D_fn(P, D, options, images, gen_images):
    ...
    real1, real2, fake = samples[:N], samples[N:2*N], samples[2*N:]
    debiased_loss = debiased_con_loss(real1, real2, fake, temperature=P.temp, distributed=P.distributed)
