...
# make a change to supervised contrastive learning
# each L_self = pos/ pos+ neg_generated
# which means the neg_real do not participate in the calculation
def DCD_loss_v4(real1, real2, fake1, fake2, temperature, params, distributed=False):
    if distributed:
        real1 = torch.cat(GatherLayer.apply(real1), dim=0)
        real2 = torch.cat(GatherLayer.apply(real2), dim=0)
    ...
    Ng = (-tau_plus * (N) * pos + neg_fake) / (1 - tau_plus)
    # constrain (optional)
...
def loss_D_fn(P, D, options, images, gen_images):
    ...
    dcd_loss = DCD_loss_v4(real1, real2, fake1, fake2, temperature=P.temp, params=P, distributed=P.distributed)
