...
def loss_D_fn(P, D, options, images, gen_images, latent_samples):
    ...
    #constrain the representation of generated image identical to latent z
    fake_representation_constrain_loss = ((latent_samples-fakes)**2).sum(axis=1).mean()

    return simclr_loss + P.lbd_a * sup_loss+fake_representation_constrain_loss, {
        "penalty": d_loss,
        "d_real": d_real.mean(),
        "d_gen": d_gen.mean(),
    }
...
def loss_G_fn(P, D, options, images, gen_images, latent_samples):
    d_gen, aux = D(P.augment_fn(gen_images), projection=True, projection2=True)

    views = aux['projection']
    views = F.normalize(views)
    fake_representation_constrain_loss = ((latent_samples - views) ** 2).sum(axis=1).mean()
    ...
    return g_loss+fake_representation_constrain_loss
    # return g_loss
