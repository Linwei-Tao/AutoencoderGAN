...
# only difference is the sample size is half
def DCD_loss_v3(real1, real2, fake1, fake2, temperature, params, distributed=False):
    if distributed:
    ...
    Ng = (-tau_plus * (N-2+N) * pos + neg_real + neg_fake) / (1 - tau_plus)
    # constrain (optional)
    Ng = torch.clamp(Ng, min=(N-2+N) * np.e ** (-1 / temperature))
...
# same size of real and generated samples (based on DCD_v3)
def DCD_loss_v5(real1, real2, fake1, fake2, temperature, params, distributed=False):
    if distributed:
        real1 = torch.cat(GatherLayer.apply(real1), dim=0)
        real2 = torch.cat(GatherLayer.apply(real2), dim=0)
        fake1 = torch.cat(GatherLayer.apply(fake1), dim=0)
    ...
    N_gg = (-tau_plus4gen * N_g * pos + neg_generate) / (1 - tau_plus4gen)
    # constrain
...
def loss_D_fn(P, D, options, images, gen_images):
    ...
    dcd_loss = DCD_loss_v5(real1, real2, fake1, fake2, temperature=P.temp, params=P, distributed=P.distributed)
