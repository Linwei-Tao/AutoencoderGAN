...
def parse_args():
    ...
    parser.add_argument('--tau_plus', default=0.1, type=float,
                        help='Prior class probability for query sample ')
...
def worker(P):
    ...
    if P.resume:
        logger = Logger(None, root=P.logdir, resume=P.resume)
    else:
        ...
        logger = Logger(f'{P.filename}_{_desc}{P.comment}', root=P.logdir, subdir=f'gan_dp/st_{P.gin_stem}/{P.architecture}')
...
if __name__ == '__main__':
    ...
    # setup loss function and filename
    P = setup(P)
    P.distributed = False
    worker(P)